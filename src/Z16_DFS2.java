...
public class Z16_DFS2 {
	...
	public static void main(String[] args) {
		...
		boolean [] visited = new boolean [8]; // 각 정점의 방문여부
		
		// 시작 정점을 하나 지정하여, 시작 정점을 방문, 스택에 시작정점을 넣고, 반복을 시작
		int v = 1; // 시작정점을 지정, 1A를 의미
		visited[v] = true; // 방문 체크 
		System.out.print(v + " ");//  방문하면 정점을 출력
		
		stack[++top] = v;// 마지막 갈림길을 스택에 저장
		// 반복 시작 : 스택이 비워질때까지 반복, 스택에서 정점을 꺼내 , 인접한&& 방문하지 않은 정점을
		// 방문 작업을 반복한다
		while(top>-1) {
			int w = -1; // 다음 정점을 저장할 변수를 미리 선언, 플레그 변수 역할도 겸한다 
			for (int i = gra[v].length-1; i >=0 ; i--) {// v정점에 인접한 정점만큼 반복
				 // 방문하지 않은 정적이면, 방문하기
				if(!visited[gra[v][i]]) {
